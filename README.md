...
data/processed/train.csv (≈1,112 天)    data/processed/test.csv (≈330 天)
    ...
    │  ├── MinMaxScaler    → 仅 fit 到验证目标开始前
    │  ├── 固定时间起点 val/test，目标区间互不重叠
    │  └── PowerDataset    → 训练窗口 step=1
    │
    v
...
python -m src.cli evaluate --model lstm --horizon 90 \
  --ckpt outputs/revised/checkpoints/lstm_h90_s42.ckpt
...
python -c "from src.config import Config; from src.evaluate import evaluate; \
  ...
  'outputs/revised/checkpoints/lstm_h90_s42.ckpt'))"
...
│    └── hcmrf_wo_DRD                                       │
│                                                         │
│  输出: outputs/revised/results/*.json                     │
└─────────────────────────────────────────────────────────┘
```
...
- 滞后和滚动统计仅在目标变量上计算
- `MinMaxScaler` 仅在训练窗口覆盖的时间段上 fit，避免使用验证和最终留出目标的信息
...
| step | 1 | 1 |
| 当前训练窗口数 | 813（1 个固定验证起点） | 228（1 个固定验证起点） |
```
...
输入 (90天 × 24特征)
         │
    ┌────▼────┐
    │ Conv1D  │  ← 局部特征编码器 (kernel=7, filters=64)
    │ Encoder │
    └────┬────┘
         │  特征: (batch, 90, 64)
         │
    ...
    │  365d: patch_size=3, num_patches=10  │  ← 粗粒度注意力
    │                                       │
    │  Patch + Linear 投影:                 │
...
| 组件 | 作用 | 不使用的代价 |
|------|------|-------------|
| **HCM** (分辨率压缩) | 365d 压缩噪声，90d 保持细粒度 | 365d 关注过多天级噪声 |
...
```
变体 A (完整 HCMRF):
  Encoder → HCM(res) → AdaptivePatch → Transformer → DRD
...
│  4. 汇总 5 轮 → mean ± std                               │
│                                                         │
│  指标在日用电量 kWh 量纲上计算 (inverse_transform 还原)    │
└─────────────────────────────────────────────────────────┘
```
...
│   ├── *.artifacts.joblib          # scaler + 窗口切分
│   └── *.manifest.json             # 配置、数据哈希、代码版本
│
├── results/
│   ├── summary.json              # 全部实验 mean±std
│   ├── ablation_summary.json     # 消融实验汇总
│   └── report_metrics.json       # 报告使用的协议与参数量
│
└── figures/
    ├── comparison_90d.png        # 多模型预测曲线对比
    ├── comparison_365d.png
    ├── ablation_mse_h365.png     # 消融 MSE 柱状图
    └── result_snapshot.png       # 正式实验结果快照
```
